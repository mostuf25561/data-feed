...
const getTableNameFromNotation = function (tableName) {
  return tableName.replace(".", "__");
};
const tmp_prefixForColumnNotation = "from_";
  ...
  createJsonTableFromJsonColumn: function (
    ...
    let entriesAsString = rules.map((entry) => {
      return (
        ...
        getTableNameFromNotation(entry.object_notation) +
        " " +
        entry.type +
        "  PATH " +
        "'$." +
    ...
    function steps(entries, tableSrc = "t1", tableTarget = "v1") {
      return [
...
module.exports = {
  ...
  createJsonTableFromJsonColumn: async function (
    ...
    try {
      ...
      await helpers.execSql(queries);
      // const res1 = await knex.raw(strQuery);
      const res = await knex.raw("select * from :tableTarget:", {
        tableTarget,
      });

...
const tmp_prefixForColumnNotation = "from.";

module.exports = {
  createJsonTableFromJsonColumn: function (
    rules,
    tableSrc = "t1",
    tableTarget = "v"
  ) {
...
${entriesAsString})
  ...
  wrapJsonTable: async function (rules, tableSrc, tableTarget, feed) {
    const strQuery = this.helpers.wrapJsonTable(
      rules,
      tableSrc,
      tableTarget,
      feed
    ...
    try {
      ...
      await privateHelpers.execSql(queries);
  ...
  createJsonTableFromJsonColumn: async function (
    ...
    const strQuery = this.helpers.createJsonTableFromJsonColumn(
      rules,
      tableFrom,
      tableTarget
    );
  ...
  storeJsonToDb: async function (entries, tableName) {
    ...
    function steps(entries, tableNameFinal = "t1") {
      return [
  ...
  clause: {
    ...
    caseClause: function (rules) {
      const convertRulesToSqlInstructionsObject = (rules) => {
        const sqlSuffixes = {};
      ...
      const sqlInstructions = convertRulesToSqlInstructionsObject(rules);
      const sqlQueries = {};
      sqlQuery = "";
      let aliasColumnName;
    ...
    wrapJsonTable: function (rules, tableSrc = "v2", tableTarget = "v3", feed) {
